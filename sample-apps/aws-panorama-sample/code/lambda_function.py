...
class people_counter(panoramasdk.base):
    ...
    def entry(self, inputs, outputs):
        ...
        for i in range(len(inputs.video_in)):
            ...
            self.model.batch(0, self.preprocess(stream.image))
            self.model.flush()
            ...
            # Process results
            # Model outputs (classes, probabilities, rectangles) are collected in the BatchSet returned by model.get_result
            # Each output is a Batch of arrays, one for each input in the batch
            # Batch only has one image; save results for that image 
            ...
            # Add text
            stream.add_label('People detected: {}'.format(self.number_people), 0, 0.9)
